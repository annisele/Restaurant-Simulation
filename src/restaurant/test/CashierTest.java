...
public class CashierTest extends TestCase
{
	...
	public void testThreeNormalCustomerScenario()
	{
		...
		System.out.println("THIRD TEST NORM TWO CUSTOMERS ");
				...
				cashier.msgCustomerOrder(waiter, customer,1, "chicken");
				cashier.msgCustomerOrder(waiter2, customer2,2, "salad");
				//check postconditions for step 1 and preconditions for step 2
				...
				assertTrue("CashierBill should contain a bill with state == customerApproached. It doesn't.",
						cashier.cashiers_list.get(0).state == OrderState.adding);
				assertTrue("CashierBill should contain a bill for cust2 with state == customerApproached. It doesn't.",
						cashier.cashiers_list.get(1).state == OrderState.adding);
				...
				assertTrue("CashierBill should contain a bill with state == customerApproached. It doesn't.",
						cashier.cashiers_list.get(0).state == OrderState.waiting);
				//System.out.println("WO"+cashier.cashiers_list.get(1).state );
				assertTrue("CashierBill should contain a bill for cust 2 with state == customerApproached. It doesn't.",
						cashier.cashiers_list.get(1).state == OrderState.waiting);
							  
				  //step 3

				  cashier.msgHereIsMoney(customer, 10.99);
				  cashier.msgHereIsMoney(customer2, 5.99);
				  assertTrue("Cashier should have logged \"Received money\" but didn't. His log reads instead: " 
		                  + cashier.log.getLastLoggedEvent().toString(), cashier.log.containsString("Recieved money"));
				  	
				//NOTE: I called the scheduler in the assertTrue statement below (to succinctly check the return value at the same time)
				  //System.out.println("WO"+cashier.cashiers_list.size() );
				assertEquals("Cashier should have 2 order in cashier's list. It doesn't.", cashier.cashiers_list.size(), 2);
				System.out.println("HO"+cashier.cashiers_list.size());
				//System.out.println("cashier.cashiers_list.get(1).payment");
				...
				System.out.println("HO"+cashier.cashiers_list.get(1).state );
				assertTrue("CashierBill should contain a bill of price = $8.99. It contains something else instead: $" 
		                 + cashier.cashiers_list.get(1).payment, cashier.cashiers_list.get(1).payment == 5.99);
				assertTrue("CashierBill should contain a bill with state == customerApproached. It doesn't.",
						cashier.cashiers_list.get(1).state == OrderState.paid);
	...
	public void testFourNormalCustomerScenario()
	{
		...
		//setUp() runs first before this test!
				...
				//check preconditions
		System.out.println("FOURTH TEST NORM TWO CUSTOMERS TWO WAITERS");
				assertEquals("Cashier should have 0 bills in it. It doesn't.",cashier.cashiers_list.size(), 0);                
				assertEquals("CashierAgent should have an empty event log before the Cashier's HereIsBill is called. Instead, the Cashier's event log reads: "
						+ Cashier.log.toString(), 0, Cashier.log.size());
	...
	public void testFiveNormalCustomerScenario()
	{
		...
		//setUp() runs first before this test!
				...
				//check preconditions
		System.out.println("FIFTH TEST NON NORM TWO CUSTOMERS ONE WAITERS ONE DOESNT HAVE ENOUGH MONEY");
				assertEquals("Cashier should have 0 bills in it. It doesn't.",cashier.cashiers_list.size(), 0);                
				assertEquals("CashierAgent should have an empty event log before the Cashier's HereIsBill is called. Instead, the Cashier's event log reads: "
						+ Cashier.log.toString(), 0, Cashier.log.size());
				...
				cashier.msgCustomerOrder(waiter, customer,1, "chicken");
				cashier.msgCustomerOrder(waiter2, customer2,2, "pizza");
				//check postconditions for step 1 and preconditions for step 2
				...
				System.out.println("HO"+cashier.cashiers_list.get(1).payment );
				assertTrue("CashierBill should contain a bill of price = $8.99. It contains something else instead: $" 
		                 + cashier.cashiers_list.get(1).payment, cashier.cashiers_list.get(1).payment == 8.99);
				assertTrue("CashierBill should contain a bill with state == customerApproached. It doesn't.",
						cashier.cashiers_list.get(1).state == OrderState.indebt);
				...
				assertEquals("Cashier should have 1 order in cashier's list of bad orders. It doesn't.", cashier.bad_orders.size(), 1);
	...
	public void testSixNormalCustomerScenario()
	{
		...
		//setUp() runs first before this test!
				...
				//check preconditions
		System.out.println("SIX TEST NON NORM TWO MARKETS CASHIER IN DEBT ");
		assertEquals("Cashier should have 0 bills in it. It doesn't.",cashier.cashiers_list.size(), 0);                
		assertEquals("CashierAgent should have an empty event log before the Cashier's HereIsBill is called. Instead, the Cashier's event log reads: "
				+ Cashier.log.toString(), 0, Cashier.log.size());
		...
		cashier.msgHereIsBill(market, 225.99);
		cashier.msgHereIsBill(market2, 500.99);
		//check postconditions for step 2 / preconditions for step 3
		  assertTrue("Cashier should have logged \"Received bill\" but didn't. His log reads instead: " 
                                + cashier.log.getLastLoggedEvent().toString(), cashier.log.containsString("Recieved bill"));
		 ...
		 // assertTrue("Cashier should have logged \"Market has been paid in full\" but didn't. His log reads instead: " 
		  ...
		  assertTrue("CashierBill should contain a bill of price = $225.99. It contains something else instead: $" 
	               + cashier.markets.get(0).bill, cashier.markets.get(0).bill == 225.99);
		  System.out.println("WW"+cashier.markets.get(1).bill);
		  assertTrue("CashierBill should contain a bill of price = $226.98. It contains something else instead: $" 
	               + cashier.markets.get(1).bill, cashier.markets.get(1).bill == 226.98);
			...
			assertTrue("Market should have logged \"Received money\" but didn't. His log reads instead: " 
		               + market2.log.getLastLoggedEvent().toString(), market.log.containsString("market has recieved money"));
			 assertTrue("CashierBill should contain a debt of price = 226.98. It contains something else instead: $" 
		               + cashier.markets.get(1).debt, cashier.markets.get(1).debt == 226.98);  
		  //step 3
